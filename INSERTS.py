...
class INSERTS:
    ...
    def update_location(self,zipcode,street,building,city,gps):
        ...
        if(self.cursor.rowcount==0):
            print("no location with suzh zipcode")
        self.conn.commit()
    ...
    def add_ride(self,username,cid,is_to_customer,begin_time,end_time,date,origin_point,destination_point,distance,cost):
        try:
            ...
            str(origin_point)+"','"+str(destination_point)+"','"+str(distance)+"','"+str(cost)+"')")
            self.conn.commit()
    ...
    def add_work_parts(self,WID,Part,Available,Cost):
        try:
            self.cursor.execute("UPDATE wparts "
                                ...
                                "Cost ='"+str(Cost)+"' WHERE WID='"+str(WID)+"' and Part='"+str(Part)+"'")
            if (self.cursor.rowcount == 0):
                self.cursor.execute("INSERT INTO wparts(WID, Part, Available, Cost) VALUES "
